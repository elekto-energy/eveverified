    ...
    <div className="py-8 md:py-12 flex flex-col items-center relative">
      ...
      >
        ...
        <motion.circle
          ...
          strokeWidth="1"
          strokeOpacity="0.3"
          animate={{ r: [coreR, coreR * 1.8], opacity: [0.5, 0] }}
        ...
        <motion.circle
          ...
          stroke="#00ff88"
          strokeWidth="1"
          animate={{ r: [coreR, coreR * 2.2], opacity: [0.3, 0] }}
        ...
        {layerKeys.map((key, i) => {
          ...
          const isThisHovered = hoveredNode === key
            ...
            <g key={key}>
              ...
              <AnimatePresence>
                {isActive && children.map((child, ci) => {
                  const cx = lx - totalChildWidth / 2 + ci * childSpacing
                  const l = line(lx, layerY, cx, childY, layerR, childR)
                  return (
                    <motion.line
                      key={child.id}
                      x1={l.x1} y1={l.y1} x2={l.x2} y2={l.y2}
                ...
                {isActive && children.map((child, ci) => {
                  ...
                  return (
                    <motion.g
                      ...
                      exit={{ opacity: 0, y: -10, transition: { duration: 0.4, delay: 0.2 } }}
                      transition={{ duration: 0.25, delay: ci * 0.05 }}
                    >
                      <circle
                        cx={cx} cy={childY} r={childR}
                        fill={isChildHovered ? `${layer.color}20` : "rgba(10,10,10,0.9)"}
                        stroke={isChildHovered ? layer.color : `${layer.color}60`}
                        strokeWidth={isChildHovered ? "2" : "1"}
                        style={{ 
                          cursor: 'pointer',
                          filter: isChildHovered ? `drop-shadow(0 0 10px ${layer.color}40)` : 'none',
      ...
      {/* Legend */}
      <div className="flex flex-wrap justify-center gap-4 mt-8 pt-8 border-t border-white/5 w-full max-w-2xl">
        {layerKeys.map((key) => {
          const layer = layers[key as keyof typeof layers]
          return (
