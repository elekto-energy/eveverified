...
// ═══════════════════════════════════════════════════════════════════════════════
// TWO-STAGE ASK — Healthcare Law
// ARCH: ARCH_TWO_STAGE_ASK_v1.md (LOCKED 2026-02-05)
// ═══════════════════════════════════════════════════════════════════════════════

const TRINITY_API = process.env.NEXT_PUBLIC_TRINITY_API ?? 'http://127.0.0.1:8000'
const SCOPE_KEY = 'legal.healthcare'

const SCOPE = {
  name: 'Vårdlagstiftning',
  icon: '🏥',
  color: '#00ff88',
  laws: [
    { id: 'sfs:2010:659', name: 'Patientsäkerhetslagen (PSL)' },
    { id: 'sfs:2017:30', name: 'Hälso- och sjukvårdslagen (HSL)' },
    { id: 'sfs:2015:315', name: 'Läkemedelslagen' },
    { id: 'sfs:2018:218', name: 'Dataskyddslagen (GDPR)' },
  ]
}
...
export default function AskLegalHealthcarePage() {
  ...
  useEffect(() => {
    async function loadStats() {
      try {
        ...
        setIndexStats({ count: 363 }) // fallback
      }
    }
    loadStats()
  }, [])
  ...
  return (
    <main className="min-h-screen bg-eve-dark">
        ...
        <div className="max-w-4xl mx-auto px-6 py-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs px-2 py-0.5 rounded bg-eve-green/10 text-eve-green border border-eve-green/30">
                    🔒 T3 VERIFIED
                  </span>
                </div>
                <div className="text-xs text-gray-500">
                  PSL + HSL + Läkemedelslagen • {indexStats?.count || '...'} sections
                </div>
              </div>
            </div>
            ...
            <div className="inline-flex flex-col items-start gap-0.5 px-4 py-2 rounded-lg 
              ...
              <span className="text-xs text-white font-medium">Two-Stage Ask</span>
              <span className="text-xs text-eve-green">Truth first. Explanation second.</span>
            </div>
          </div>
        </div>
      </div>
          ...
          >
            <div className="relative">
              <input
                ...
                placeholder="Fråga om vårdlagstiftning — t.ex. lex maria, vårdgivarens ansvar"
                className="w-full px-6 py-4 rounded-xl bg-black/40 border border-white/10 
                          text-white placeholder-gray-500 focus:outline-none focus:border-eve-green/50
                          transition-colors"
              />
              <button
                type="submit"
                ...
                className="absolute right-2 top-1/2 -translate-y-1/2 px-4 py-2 rounded-lg
                          bg-eve-green/20 border border-eve-green/40 text-eve-green text-sm
                          hover:bg-eve-green/30 transition-colors disabled:opacity-50"
              >
        ...
        <section className="pb-4 px-6">
          <div className="max-w-3xl mx-auto">
            ...
            >
              ...
              {/* SCOPE_VERIFIED - Legal scopes: scope exists with verified content */}
              {instantResult?.status === 'SCOPE_VERIFIED' && instantResult.verified_scope && (
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-eve-green" />
                    <span className="text-eve-green font-medium">SCOPE VERIFIERAD</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-eve-green font-medium">{instantResult.verified_scope.trust_tier}</div>
                    </div>
                    <div>
                  ...
                  <div className="p-3 rounded-lg bg-eve-green/5 border border-eve-green/20">
                    <p className="text-xs text-eve-green">
              ...
              {/* VERIFIED - Exact ref match (for medical/drugs) */}
              {instantResult?.status === 'VERIFIED' && instantResult.verified && (
                <div className="space-y-4">
                  <div className="flex items-center gap-2">
                    <span className="w-2 h-2 rounded-full bg-eve-green" />
                    <span className="text-eve-green font-medium">VERIFIERAD I SCOPE</span>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      ...
                      <div className="text-eve-green font-mono text-sm">{instantResult.verified.eve_id}</div>
                    </div>
                    <div>
                      <div className="text-xs text-gray-500">Källa</div>
                        ...
                        {witnessResult.sources.map((s, i) => (
                          <div 
                            ...
                            className={`p-3 rounded-lg bg-black/30 border transition-all cursor-pointer
                                      ${expandedSource === i ? 'border-eve-green/50' : 'border-white/5 hover:border-eve-green/30'}`}
                          ...
                          >
                            <div className="flex items-start justify-between">
                              <div className="flex-1">
                                ...
                                <div className="text-xs text-eve-green font-mono mt-1 ml-7">{s.ref}</div>
                              </div>
                            ...
                            {expandedSource === i && (
                              <div className="mt-3 ml-7 space-y-3">
                                <div className="p-3 rounded bg-black/40 border border-eve-green/20">
                                  <div className="text-xs text-eve-green mb-2 font-medium">📜 Verifierad lagtext</div>
                                ...
                                <div className="grid grid-cols-2 gap-2 text-xs">
                                  <div className="p-2 rounded bg-black/30">
                                    ...
                                    <span className="text-eve-green font-mono ml-1">{s.eve_id}</span>
                                  </div>
                                  <div className="p-2 rounded bg-black/30">
                                ...
                                <div className="flex items-center gap-1 text-eve-green text-xs">
                                  <span>🔒</span>
      ...
      <section className="pb-32 px-6">
        <div className="max-w-3xl mx-auto">
          <div className="text-xs text-gray-500 mb-4">Lagar i scope</div>
          <div className="grid grid-cols-2 gap-3">
            {SCOPE.laws.map((law, i) => (
              <div key={i} className="p-3 rounded-lg bg-black/30 border border-white/5">
                <div className="text-sm text-white">{law.name}</div>
                <div className="text-xs text-eve-green font-mono mt-1">{law.id}</div>
              </div>
            ))}
          </div>
        </div>
